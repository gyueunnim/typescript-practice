function typeNarrowing(x : number|string) :(number|string) {
    if(typeof x === 'number') 
        return x + 1;
    else
        return x + '1'
...
function typeAssertion(x :number|string) {
    let array :number[] = [];
    array[0] = x as number; // x를 number로 인식 그러나 실제로 타입을 바꿔주지는 않는다.(주장만)
}
/*
...
function cleaning2(a :(string|number)[]) :number[] {
    let array :number[] = [];
    a.forEach((b) => {
        if(typeof b === "string") {
